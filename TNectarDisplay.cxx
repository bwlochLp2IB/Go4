...
void TVmmrSlaveDisplay::InitDisplay(UInt_t parentboard, Bool_t replace)
{
  if (replace)
    ...
    /*
     ...
     //hRawADC16vsallhorizontal
     /*
     // 1ch Vertical vs all horizontal 
    for(int i=0;i<16;i++){
     obname.Form("Raw/VMMR/Board_%d/Slave_%d/VMMR_ADC_V%dvsH_%d_%d", parentboard, brd, i,parentboard, brd);
     obtitle.Form("VMMR Board %d Frontend %d : ADC (Vertical strip) %d vs all horizontal map)", parentboard, brd,i);     
     hRawADCVvsH[i]= MakeTH2('I', obname.Data(), obtitle.Data(), VMMR_ADC_RANGE/0xF, 0, VMMR_ADC_RANGE,  VMMR_ADC_RANGE/0xF, 0, VMMR_ADC_RANGE,"All horizontal strips", "ADC");     
        } */
     /*
    // 1ch Horizontal vs all vertical    
    for(int i=0;i<16;i++){
     obname.Form("Raw/VMMR/Board_%d/Slave_%d/VMMR_ADC_H%dvsV_%d_%d", parentboard, brd, i,parentboard, brd);
     obtitle.Form("VMMR Board %d Frontend %d : ADC (horizontal strip) %d vs all vertical map)", parentboard, brd,i);     
     hRawADCHvsV[i]= MakeTH2('I', obname.Data(), obtitle.Data(), VMMR_ADC_RANGE/0xF, 0, VMMR_ADC_RANGE,  VMMR_ADC_RANGE/0xF, 0, VMMR_ADC_RANGE,"All vertical strips", "ADC");     
        }  
    */
     /*
     hSTRIPmap= MakeTH2('I', "BB8 Strip Mapping (MMR ch to strip channel)", " a " , 16, 0.5 , 16.5 , 16, -16.5 , -0.5 , "V - strips" , "H - strips");  */
